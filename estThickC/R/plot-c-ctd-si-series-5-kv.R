...
c.to.si.loess.5kV <- loess(c.to.si.mu~t.C.nm, df)
print(summary(c.to.si.loess.5kV))
fi <- '../dat/rda/c-to-si-loess-5kV.rda'
save(c.to.si.loess.5kV, file=fi)
...
fi <- '../pdf/c-ctd-si-series-plt-5kV.pdf'
ggsave(ctosiInt, file=fi, width=9.0, height=6.0, units="in", dpi=300)

fi <- '../png/c-ctd-si-series-plt-5kV.png'
