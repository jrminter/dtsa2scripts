...
fi <-'../dat/csv/C-ctd-Si-20-kV-10000-Traj.csv'
...
c.to.si.loess.20kV <- loess(c.to.si.mu~t.C.nm, df)
print(summary(c.to.si.loess.20kV))
fi <- '../dat/rda/c-to-si-loess-20kV.rda'
save(c.to.si.loess.20kV, file=fi)
...
ctosiInt <- ggplot(df2, aes(x=t.C.nm, y=c.to.si.mu)) + 
    ...
    ggtitle("Monte Carlo Model of C-coatings on Si at 20 kV") +
