...
strCsv <- '../csv/sim-C-on-Cu-10kV.csv'
...
do.plot <- function(){
  ...
  text(350,0.15,"Monte Carlo simulation of C on Cu at 10 kV",
       cex=0.8)
  ...
  text(350,0.9,"Monte Carlo simulation of C on Cu at 10 kV",
       cex=0.8)
...
c.on.cu.10kV <- data.frame(tNmC=df$tNm, kCKa=kCKa.predict,
                          kCuLa=kCuLa.predict)

strRD <- '../RData/c.on.cu.10kV.RData'
save(c.on.cu.10kV, file=strRD)

do.plot()

if(bSavePlots==TRUE){
  library(rEDS)
  rsPlotPng(do.plot(), "../plt/c-on-cu-10kV.png", width = 7.5,
            height = 5, pts = 16, dpi = 100)
  rsPlotPdf(do.plot(), "../plt/c-on-cu-10kV.pdf", width = 9,
            height = 6, pts = 12)
}
