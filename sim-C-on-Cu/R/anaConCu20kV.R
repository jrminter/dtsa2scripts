...
strCsv <- '../csv/sim-C-on-Cu-20kV.csv'
...
do.plot <- function(){
  ...
  text(350,0.15,"Monte Carlo simulation of C on Cu at 20 kV",
       cex=0.8)
  ...
  text(350,0.9,"Monte Carlo simulation of C on Cu at 20 kV",
       cex=0.8)
...
c.on.cu.20kV <- data.frame(tNmC=df$tNm, kCKa=kCKa.predict,
                          kCuLa=kCuLa.predict)

strRD <- '../RData/c.on.cu.20kV.RData'
save(c.on.cu.20kV, file=strRD)

do.plot()

if(bSavePlots==TRUE){
  library(rEDS)
  rsPlotPng(do.plot(), "../plt/c-on-cu-20kV.png", width = 7.5,
            height = 5, pts = 16, dpi = 100)
  rsPlotPdf(do.plot(), "../plt/c-on-cu-20kV.pdf", width = 9,
            height = 6, pts = 12)
}
