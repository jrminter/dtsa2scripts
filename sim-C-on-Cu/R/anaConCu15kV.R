...
strCsv <- '../csv/sim-C-on-Cu-15kV.csv'
...
do.plot <- function(){
  ...
  text(350,0.15,"Monte Carlo simulation of C on Cu at 15 kV",
       cex=0.8)
  ...
  text(350,0.9,"Monte Carlo simulation of C on Cu at 15 kV",
       cex=0.8)
...
c.on.cu.15kV <- data.frame(tNmC=df$tNm, kCKa=kCKa.predict,
                          kCuLa=kCuLa.predict)

strRD <- '../RData/c.on.cu.15kV.RData'
save(c.on.cu.15kV, file=strRD)

do.plot()

if(bSavePlots==TRUE){
  library(rEDS)
  rsPlotPng(do.plot(), "../plt/c-on-cu-15kV.png", width = 7.5,
            height = 5, pts = 16, dpi = 100)
  rsPlotPdf(do.plot(), "../plt/c-on-cu-15kV.pdf", width = 9,
            height = 6, pts = 12)
}
