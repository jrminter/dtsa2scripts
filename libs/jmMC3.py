...
#						   and cleaned up examples. NOTE: use tabs not spaces...
#                          Note the .1 - this is after spaces cleanup...
# 2020-08-12  JRM  0.0.21  Added bareBulkStd. This only computes a spectrum
#                          for a bare bulk standard. 

__revision__ = "$Id: jmMC3.py John Minter $"
__version__ = "0.0.21 2020-08-12"
...
def bareBulkStd(mat, det, e0, nTraj, lt=100, pc=1.0):
	"""
	bareBulkStd(mat, det, e0, nTraj, lt=100, pc=1.0)

	Use mc3 simulation to simulate an uncoated standard specimen.
	Only generate a spectrum.
	...
	a = bareBulkStd(mat, det, e0, nTraj, lt=100, pc=1.0)
	a.display()
	"""
	start = time.time()
	strMat = mat.getName()
	...
	sim = mc3.simulate(mat, det, e0, lt*pc, True, nTraj, True, True, xtraParams)
...
def simCoatedSubstrate(mat, ctg, thNm, det, e0, nTraj, lt=100, pc=1.0):
	...
	sim = mc3.multiFilm(layers,det,e0,True,nTraj,dose,True,True,xtraParams)
	sName = "%g-nm-%s-on-%s-%g-kV" % (thNm, ctg, mat, e0)
	sim.rename(sName)
	return sim

def simCarbonCoatedMaterial(mat, det, e0, nTraj, lt=100, pc=1.0, tc=20.0):
	"""simCarbonCoatedMaterial(mat, det, e0, nTraj, lt=100, pc=1.0, tc=20.0)
	...
	"""
	dose = pc * lt  # na-sec"
	amc = dt2.material("C", density=1.35)
...
def simBulkStd(mat, det, e0, nTraj, lt=100, pc=1.0, ctd=True):
	...
	Note: This ONLY produces a spectrum
