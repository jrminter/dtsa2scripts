...
__revision__ = "$Id: __init__.py Nicholas W. M. Ritchie 2016-07-15$"
__version__ = "0.0.7"
...
class ScriptableSpectrum(epq.BaseSpectrum):
    ...
    def sumCounts(self, ch0, ch1):
        ...
        res = epq.SpectrumUtils.sumCounts(self.wrapped, ch0, ch1)
    ...
    def integrate(self, e0, e1):
        ...
        res = epq.SpectrumUtils.integrate(self.wrapped, e0, e1)
...
def ionizationRange(mat, e0=20.0, alg=epq.ElectronRange.KanayaAndOkayama1972):
    """ionizationRange(mat,[e0=20.0],[alg=epq.ElectronRange.KanayaAndOkayama1972])
    Ex: ionizationRange(createMaterial(), 30.0)
...
def compare(item, std=None):
    ...
    3) if item is a list creates a table comparing the Composition objects or the MicroanalyticalComposition property to the nominal value."""
    def sss(a):
    ...
    if isinstance(item, epq.ISpectrumData):    
        ...
        elms = ju.TreeSet()
        tmp = "<table><tr><th>Element</th><th>Nominal</th>"
        for ii in item:
            if not isinstance(std, epq.Composition):
                std = ii.getProperties().getCompositionProperty(epq.SpectrumProperties.StandardComposition)
            tmp = "%s<th colspan=2>%s</th>" % (tmp ,ii,)
            if not isinstance(ii, epq.Composition):
                ii = ii.getProperties().getCompositionProperty(epq.SpectrumProperties.MicroanalyticalComposition)
            elms.addAll(ii.getElementSet())
        tmp = "%s</tr>" % (tmp, )         
        for elm in elms:
            tmp = "%s\n<tr><td>%s</td><td>%0.3g</td>" % (tmp, elm.toAbbrev(), std.weightFraction(elm, False), )
            for ii in item:
                if not isinstance(ii, epq.Composition):
                    ii = ii.getProperties().getCompositionProperty(epq.SpectrumProperties.MicroanalyticalComposition)
                    tmp = "%s<td>%0.3g</td><td>%0.3g</td>" % (tmp, ii.weightFraction(elm, False), std.weightFraction(elm, False)-ii.weightFraction(elm, False)) 
            tmp = "%s</tr>" % (tmp, )
        tmp = "%s</table>" % (tmp, )
        report(tmp)
...
def tabulate(specs, withErrs=False, normalize=False, prop=epq.SpectrumProperties.MicroanalyticalComposition, precision=4, massFrac=True, asOxides=False):
    """tabulate(specs,[withErrs=False],[normalize=False],[prop=epq.SpectrumProperties.MicroanalyticalComposition],[precision=4],[massFrac=True])
    ...
    additional decimal digits of precision. [massFrac->False for atomic fraction.]"""
    if isinstance(prop, str):
        propMap = { 
             'k' : epq.SpectrumProperties.KRatios,
             'c' : epq.SpectrumProperties.MicroanalyticalComposition,
