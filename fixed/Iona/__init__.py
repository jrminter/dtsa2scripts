...
def multiQuant(det, e0, stds, refs={}, preferred=(), elmByDiff=None, oByStoic=False, oxidizer=None):
   """multiQuant(det, e0, stds, refs={}, preferred=(), oByStoic=False, oxidizer=None)
...
def quantify(unknown, stds, refs={}, preferred=(), elmByDiff=None, oByStoic=False, oxidizer=None):
    """quantify(unk, stds, refs={}, preferred=(), elmByDiff = None, oByStoic=False, oxidizer=None)
...
+ oxidizer - (optional) An epq.Oxidizer object to use in place of the default set"""
    ...
    qus = multiQuant(det, e0, stds, refs, preferred, elmByDiff, oByStoic, oxidizer)
...
def saveSpectra(specs, path=None):
    ...
    if isinstance(specs, epq.ISpectrumData):
        ...
        fPth = "%s\\%s.msa" % (path, name)
        fPth = fPth.replace("\\", "/")
        wrap(specs).save(fPth)
